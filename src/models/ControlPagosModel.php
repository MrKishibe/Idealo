...
class ControlPagosModel extends Database {
    ...
    protected function validar(&$datos, $esEdicion = false) {
        if ($esEdicion && empty($datos['id_pago'])) {
            throw new Exception("El ID del pago es obligatorio.");
        }
        if (empty($datos['id_pedido'])) {
            throw new Exception("El número de pedido es obligatorio.");
        }
        if (empty($datos['monto_pago']) || !is_numeric($datos['monto_pago']) || $datos['monto_pago'] <= 0) {
            throw new Exception("El monto debe ser válido y mayor a 0.");
        }
        
        // Validación estricta de referencia (Obligatoria, solo números, 6 caracteres exactos)
        if (empty($datos['referencia']) || !preg_match('/^[0-9]{6}$/', $datos['referencia'])) {
            throw new Exception("La referencia es obligatoria y debe contener exactamente 6 números.");
        }
    }
    ...
    public function guardarPago($datos) {
        ...
        return $this->pdo->connect()->prepare($sql)->execute([':id_pedido' => $datos['id_pedido'], ':monto_abonado' => $datos['monto_pago'], ':id_metodo_de_pago' => $datos['id_metodo_de_pago'], ':referencia' => $datos['referencia'], ':fecha_pago' => $datos['fecha_pago']]);
    }
    ...
    public function editarPago($datos) {
        ...
        return $this->pdo->connect()->prepare($sql)->execute([':id_pedido' => $datos['id_pedido'], ':monto_abonado' => $datos['monto_pago'], ':id_metodo_de_pago' => $datos['id_metodo_de_pago'], ':referencia' => $datos['referencia'], ':fecha_pago' => $datos['fecha_pago'], ':id_pago' => $datos['id_pago']]);
    }
